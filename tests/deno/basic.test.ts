...
Deno.test("LibRSVG initialization", async () => {
  ...
  try {
    ...
    assert(error.message.includes("Failed to load"), "Should fail with loading error")
  }
})
...
Deno.test("SVG capabilities detection", async () => {
  ...
  try {
    ...
    assert(error.message.includes("not initialized") || error.message.includes("Failed to load"))
  }
})
...
Deno.test("Error handling for uninitialized operations", async () => {
  ...
  const testSvg = '<svg width="100" height="100"><rect width="50" height="50" fill="red"/></svg>'

  try {
    await librsvg.render(testSvg)
    assert(false, "Should throw error when not initialized")
  } catch (error) {
    assert(error.message.includes("not initialized"), "Should indicate library not initialized")
  }
