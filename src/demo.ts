...
import { createLibRSVG, LibRSVG, SVGColorspace } from "./lib/index.js";
...
async function svgDemo() {
  ...
  try {
    ...
    console.log(
      `  Max Image Size: ${capabilities.maxImageSize}x${capabilities.maxImageSize}`,
    );
    ...
    for (let i = 0; i < testSVGs.length; i++) {
      ...
      for (const config of renderConfigs) {
        ...
        try {
          ...
          console.log(
            `    💾 Memory: ${(rendered.memoryUsage / 1024).toFixed(1)}KB`,
          );
          ...
          console.log(
            `    🎨 Sample pixel: RGBA(${samplePixel.r},${samplePixel.g},${samplePixel.b},${samplePixel.a})`,
          );
        } catch (error) {
    ...
    console.log(
      `  Total memory usage: ${(metrics.memoryUsage / 1024).toFixed(1)}KB`,
    );
