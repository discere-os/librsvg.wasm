...
export interface LibRSVGModule extends EmscriptenModule {
  ...
  _rsvg_handle_new_from_data(data: number, dataSize: number, error: number): number
  ...
  _cairo_image_surface_create(format: number, width: number, height: number): number
  ...
  _rsvg_wasm_render_to_rgba(svgData: number, svgLen: number, width: number, height: number): number
  _rsvg_wasm_get_svg_info(svgData: number, svgLen: number, width: number, height: number): number
