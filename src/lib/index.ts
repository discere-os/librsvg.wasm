...
export class LibRSVG {
  ...
  async initialize(wasmModule?: LibRSVGModule): Promise<void> {
    ...
    this.simdSupported =
      this.module.ccall("rsvg_wasm_get_simd_support", "number", [], []) === 1;
  ...
  render(
    ...
    try {
      ...
      const infoResult = this.module!.ccall(
        "rsvg_wasm_get_svg_info",
        "number",
      ...
      if (options.width || options.height) {
        this.module!.ccall("rsvg_wasm_set_viewport", "void", [
          "number",
          "number",
        ], [width, height]);
      }
      ...
      if (!outputPtr) {
        ...
        const errorMsg = this.module!.ccall(
          "rsvg_wasm_get_last_error",
          "string",
          [],
          [],
        );
      ...
      renderedData.set(
        this.module!.HEAPU8.subarray(outputPtr, outputPtr + outputSize),
      );
   ...
   */
  getImageInfo(svgData: string | Uint8Array): SVGImageInfo {
    if (!this.isInitialized) {
      throw new Error("LibRSVG not initialized. Call initialize() first.");
    }
  ...
  private loadModuleFactory(): Promise<() => LibRSVGModule> {
  ...
  private shouldUseSideModule(): boolean {
    ...
    return typeof (globalThis as { Module?: GlobalModule }).Module?.externalWebGPUContext !== "undefined" &&
      typeof globalThis.Deno === "undefined" &&
      !this.loadingOptions.forceMainModule;
  }

  private async loadSideModuleViaOrchestrator(): Promise<() => LibRSVGModule> {
    // Request SIDE_MODULE loading from the orchestrator
    const orchestrator = (globalThis as { Module?: GlobalModule }).Module?.externalWebGPUContext;
    ...
    try {
      ...
      console.warn(
        "SIDE_MODULE loading failed, falling back to MAIN_MODULE:",
        error,
      );
      return this.loadMainModuleFactory();
    }
  ...
  private async loadMainModuleFactory(): Promise<() => LibRSVGModule> {
    // Deno-first development environment
    if (typeof globalThis.Deno !== "undefined") {
      try {
        const moduleFactory = (await import("../../librsvg-main.js")).default as () => LibRSVGModule;
    ...
    for (const url of cdnUrls) {
      try {
        const moduleFactory = (await import(`${url}librsvg-main.js`)).default as () => LibRSVGModule;
        return moduleFactory;
      } catch {
        continue;
      }
  ...
  private async loadWasmBinary(): Promise<ArrayBuffer | undefined> {
    ...
    if (typeof globalThis.Deno !== "undefined") {
      try {
        const wasmPath =
          new URL("../../librsvg-main.wasm", import.meta.url).pathname;
...
export async function createLibRSVG(
  options?: Partial<SVGLoadingOptions>,
): Promise<LibRSVG> {
