...
import { LibRSVG, createLibRSVG } from './lib/index.js'
...
async function runBenchmarks() {
  ...
  try {
    ...
    console.log(`⚡ WebGPU Support: ${capabilities.webgpuSupport ? 'Enabled' : 'Disabled'}`)
    ...
    for (const test of testComplexities) {
      ...
      for (let i = 0; i < test.iterations; i++) {
        ...
        if (i === 0) {
          // Verify first render
          console.log(`  🎨 First render: ${rendered.width}x${rendered.height}, ${rendered.data.length} bytes`)
        }
      }

      const totalRenderTime = performance.now() - renderStart
      const avgRenderTime = renderResults.reduce((a, b) => a + b, 0) / renderResults.length
      ...
      // Memory stress test
      console.log(`  🧠 Memory stress test (${test.iterations} concurrent renders)...`)
      ...
      console.log(`  ⚡ Concurrent renders completed in ${memTime.toFixed(2)}ms`)
      console.log(`  💾 Average time per render: ${(memTime / test.iterations).toFixed(2)}ms`)
    }
    ...
    for (const scaleTest of scaleTests) {
      ...
      console.log(`  📐 ${scaleTest.name}: ${scaledWidth}x${scaledHeight} in ${scaleTime.toFixed(2)}ms`)
    }
    ...
    console.log(`  Total Memory Used: ${(metrics.memoryUsage / 1024 / 1024).toFixed(2)}MB`)
...
function createMediumSVG(): string {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if (i % 3 === 0) {
      ...
      shapes.push(`<rect x="${x-20}" y="${y-20}" width="40" height="40" fill="${fill}"/>`)
    } else {
      shapes.push(`<polygon points="${x-20},${y+20} ${x},${y-20} ${x+20},${y+20}" fill="${fill}"/>`)
    }
  }
