...
const MEDIUM_SVG = `<svg width="200" height="200" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
const COMPLEX_SVG = `<svg width="400" height="400" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  ${Array.from({length: 50}, (_, i) =>
    `<circle cx="${50 + (i % 8) * 40}" cy="${50 + Math.floor(i / 8) * 40}" r="15"
     fill="hsl(${i * 7}, 70%, 50%)" opacity="0.7"/>`
  ).join('\\n')}
...
try {
  ...
  console.log("LibRSVG initialization failed - benchmarks will show expected errors")
  lib = new LibRSVG() // Create instance anyway for structure testing
}

// Benchmark: Simple SVG rendering
Deno.bench("Simple SVG (100×100, solid fill)", { group: "svg-rendering" }, async () => {
  if (!lib) return
...
Deno.bench("Medium SVG (200×200, gradient + shapes)", { group: "svg-rendering" }, async () => {
  if (!lib) return
...
Deno.bench("Complex SVG (400×400, 50+ elements)", { group: "svg-rendering" }, async () => {
  if (!lib) return
...
Deno.bench("SVG info extraction (parse only)", { group: "operations" }, async () => {
  if (!lib) return
